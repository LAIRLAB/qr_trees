...
inline Vector<udim> iLQRHindsightValueSolver<xdim,udim>::compute_first_control(const Vector<xdim> &x0) const
{    
...
template<int xdim, int udim>
inline Vector<udim> iLQRHindsightValueSolver<xdim,udim>::compute_control_stepsize(
...
template<int xdim, int udim>
inline double iLQRHindsightValueSolver<xdim,udim>::forward_pass(const int branch_num,
...
template<int xdim, int udim>
inline void iLQRHindsightValueSolver<xdim,udim>::solve(const int T, 
        ...
        const bool warm_start, const int t_offset)
{
    ...
    for (iter = 0; iter < max_iters; ++iter)
    {
        ...
        for (int branch_num = 0; branch_num < num_branches; ++branch_num)
        {
            ...
            const Matrix<xdim,xdim> &Vt1 = branch.Vs[1];
            const Matrix<1,xdim> &Gt1 = branch.Gs[1];
...
template<int xdim, int udim>
inline void iLQRHindsightValueSolver<xdim,udim>::bellman_backup(
        ...
        )
{
    ...
    // This does not affect the policy but is part of the value function.
    const double c = branch.cost(x, u, t);
    const Vector<xdim> tmp2 = B*kt;
    Matrix<1,1> Wt_mat = Gt1*B*kt + 0.5*(tmp2.transpose()*Vt1*tmp2);
    Wt = Wt_mat(0) + Wt1 + c;
}

template<int xdim, int udim>
inline int iLQRHindsightValueSolver<xdim,udim>::timesteps() const
{ 
...
template<int xdim, int udim>
inline void iLQRHindsightValueSolver<xdim,udim>::set_branch_probability(const int branch_num, const double probability)
{
    IS_BETWEEN_LOWER_INCLUSIVE(branch_num, 0, branches_.size());
...
template<int xdim, int udim>
inline double iLQRHindsightValueSolver<xdim,udim>::total_branch_probability()
{
    double total_prob = 0;
...
template<int xdim, int udim>
inline double iLQRHindsightValueSolver<xdim, udim>::compute_value(const int branch_num, const Vector<xdim> &xt, const int t) const
{
    IS_BETWEEN_LOWER_INCLUSIVE(branch_num, 0, branches_.size());
    const int T = timesteps();
    ...
    const HindsightBranchValue<xdim,udim> &branch = branches_[branch_num];
    // Value function is actually defined in terms of the state-difference 
    // from the Taylor expansion point.
    const Vector<xdim> zt = (xt - branch.xhat[t]);
